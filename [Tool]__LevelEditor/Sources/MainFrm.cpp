...
BEGIN_MESSAGE_MAP(CLevelMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_FILE_TOOLBAR_CAMTO, OnFileCamMoveTo)
END_MESSAGE_MAP()
...
CLevelMainFrame::CLevelMainFrame()
{
	...
	m_pGLGaeaServer = &GLGaeaServer::GetInstance();
}

CLevelMainFrame::~CLevelMainFrame()
{
	if (m_pGLGaeaServer != NULL)
		SAFE_DELETE(m_pGLGaeaServer);
...
int CLevelMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndDialogBar.SetBarStyle ( dwStyle | CBRS_TOOLTIPS | CBRS_SIZE_DYNAMIC );
...
void CLevelMainFrame::OnFileToolbarAll90()
{
	...
	{
		GLLandMan* pGLLandMan = m_pGLGaeaServer->GetRootMap();
		pGLLandMan->SetObjRotate90();
	}
}

void CLevelMainFrame::OnFileCamMoveTo() 
{
	CLevelEditorView* pView = (CLevelEditorView*)GetActiveView();
	pView->SetCharMoveTo();
}
